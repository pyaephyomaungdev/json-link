...
- **First-Class Vite Starter (ZIP)**: Export your translation project as a ready-to-run React 19 + Vite 6 + TypeScript application archive with one click.
...
- **Ultra-Lightweight Client Loader (`i18n.ts`)**: Under 50 lines, zero external npm dependencies, reactive language state switching, and parameter interpolation (`{name}`).
...
- **Embedded Devtools Drawer (`<JsonLinkDevtools />`)**: Bundled slide-over drawer for in-browser key search, live in-memory copy tweaking with instant re-render, and 1-click JSON export (zero backend daemons required).
...
- **Official AI Skill**: Includes `.agents/skills/jsonlink-vite-i18n/SKILL.md` directly in the repository to guide AI coding assistants (Cursor, Claude Desktop, Antigravity) to scaffold and wire translations automatically.
